...
#Script orginally written by Jesse Breinholt (jessebreinholt@gmail.com) and Chandra Earl (sunray1@ufl.edu) for Resolving relationships among the megadiverse butterflies and moths with a novel pipeline for Anchored Phylogenomics. 2018. Breinholt JW, Earl C, Lemmon AR, Lemmon EM, Xiao L Kawahara AY. Systematic Biology, Volume 67, Issue 1
...
# this has been modified by Jesse Breinholt (jessebreinholt@gmail.com) to handel protein refrence sequences
...
#dropped evalues and targe_ cov
blast_command_ntaa = "usearch -ublast %s -db %s -evalue 1e-4 -userout %s -userfields query -threads %s -strand both"
blast_command_ntnt = "usearch -ublast %s -db %s -evalue 1e-10 -userout %s -userfields query -threads %s -strand both"
#changed output of blast_command_final
blast_command_final = "usearch -ublast %s -db %s -evalue 1e-4 -target_cov 0.40 -top_hit_only -matched %s -userout %s -userfields query+target+id+alnlen+mism+opens+qlo+qhi+tlo+thi+evalue+bits+orfframe+orflo+orfhi	-threads %s -strand both -qsegout %s"
...
bridger_command_cov = "Bridger.pl --seqType fq --left %s --right %s --clean --CPU %s --min_kmer_coverage %s --output %s --pair_gap_length %s --kmer_length %s"
bridger_command = "Bridger.pl --seqType fq --left %s --right %s --clean --CPU %s --output %s --pair_gap_length %s --kmer_length %s"
blast_filter = "usearch -ublast %s -db %s -evalue 1e-4 -target_cov 0.40 -threads %s -matched %s -strand both"
...
if hflag:
	sys.exit("\n\n#################################################################\n\n\t\tIteritive Baited Assembly IBA\
			 \n\n\n\t-raw1\tfirst raw data file\n\t-raw2\tsecond raw data file\n\t-d\tdirectory where protien reference fasta files are located\
...
# removed as this output is amino acid and often is trimmed short of the entire coding region	
...
print("#################\nprotien_IBA Complete beep.. beep.. beep...\n\n\n\t\t ^ ^\n\t\t @ @\n\t\t  $\n\t       MMMMMMM\n\t       M <O> M\n\t       M     M\n\n\t   Have a Nice Day!\n\n")
sys.exit()
